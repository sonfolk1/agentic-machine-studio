      ...
      <div className="flex-1 flex justify-center items-center gap-2 text-[12.5px] text-ink-300/90">
        {(model || isCustom) && (
          <span className="no-drag inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-ink-800/70 border border-white/[0.05]">
            <span className="text-ink-200/70">
              {model ? <BrandIcon brand={model.group.brand} size={11} /> : <IconSliders size={11} />}
            </span>
            <span className="font-medium tracking-tight max-w-[220px] truncate">
              {model ? model.entry.label : settings.selectedModel}
            </span>
          </span>
        )}
